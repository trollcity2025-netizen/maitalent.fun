...
export default function MaiPayOverview() {
  ...
  const { wallet, loadWallet, hypeCoins, giftCardProgress } = useWalletStore();
  ...
  const balanceCards = [
    ...
    { label: 'Cash Balance', value: `$${(wallet?.total_won ?? 0).toFixed(2)}`, color: 'neon-text-yellow', icon: '💰' },
    { label: 'Reserved', value: user?.cashout_reserved_coins ?? 0, color: 'neon-text-yellow', icon: '🔒' },
    { label: 'Crowns', value: user?.battle_crowns ?? 0, color: 'neon-text-purple', icon: '👑' },
    { label: 'Gift Card Progress', value: giftCardProgress, color: 'neon-text-pink', icon: '🎁' },
  ];

  const infoCards = [
    {
      title: 'How MAI Pay Works',
      text: 'Convert your Troll Coins to cash and withdraw via PayPal. A 2.9% PayPal processing fee applies.',
    },
    {
      title: 'Crown Conversion',
      text: 'Convert Battle Crowns (100 Crowns → 500 Troll Coins) to boost your cashout balance.',
    },
    {
      title: 'Processing Time',
      text: 'Approved payouts are processed within 30 Minutes to your PayPal account.',
    },
  ];
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '16px', marginBottom: '32px' }}>
        ...
        <div className="card neon-border-pink" style={{ textAlign: 'center', padding: '20px' }}>
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.75rem', marginBottom: '4px' }}>Pending Cashouts</p>
          <p className="neon-text-yellow" style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: '1.5rem' }}>
            {pendingCashouts}
          </p>
        </div>
        <div className="card neon-border-pink" style={{ textAlign: 'center', padding: '20px' }}>
      ...
      <div style={{ textAlign: 'center' }}>
        <Link to="/mai-pay" className="btn btn-green" onClick={() => {}}>
          Go to Cash Out →
        </Link>
      </div>
    </div>
