...
import { Coins, Zap, Gamepad2, Clock, Wallet, Flame, Gift } from 'lucide-react';
import SEO from '../components/SEO';

export default function DashboardPage() {
  const { user, loading: authLoading } = useAuthStore();
  const { wallet, loadWallet, loading: walletLoading, tokenTransactions, loadTokenTransactions, hypeCoins, giftCardProgress } = useWalletStore();
  ...
  const cashBalance = (wallet?.total_won ?? 0);
  ...
  const stats = [
    ...
    { label: 'Gift Card', value: `${(giftCardProgress ?? 0).toFixed(0)}%`, icon: Gift, color: 'var(--accent-purple)' },
  ];
