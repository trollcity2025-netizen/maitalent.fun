...
import { Menu, X, Coins, LogOut, Shield, Home, Gamepad2, ShoppingBag, CreditCard, LayoutDashboard, Wallet, Flame, Gift } from 'lucide-react';
...
export default function Navbar() {
  ...
  const giftCardProgress = useWalletStore((state) => state.giftCardProgress);
        ...
        <div style={{ display: 'none', alignItems: 'center', gap: '10px' }} className="desktop-actions">
          {user && wallet && (
            <div style={{
              ...
              <div style={{ width: '1px', height: '16px', background: 'rgba(255,255,255,0.1)' }} />
              <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                <Gift size={14} style={{ color: 'var(--accent-purple)' }} />
                <span style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--accent-purple)' }}>
                  {giftCardProgress.toFixed(0)}%
                </span>
              </div>
            </div>
          )}
            ...
            {user && wallet && (
              <div style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <span style={{ color: 'var(--accent-gold)', fontWeight: 700 }}>${(wallet.total_won ?? 0).toFixed(2)}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ color: 'var(--text-muted)', fontSize: '0.8rem' }}>Gift Card</span>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                    <div style={{ width: '60px', height: '8px', borderRadius: '4px', background: 'rgba(255,255,255,0.1)', overflow: 'hidden' }}>
                      <div style={{ width: `${Math.min(giftCardProgress, 100)}%`, height: '100%', background: 'var(--accent-purple)', borderRadius: '4px' }} />
                    </div>
                    <span style={{ color: 'var(--accent-purple)', fontWeight: 700, fontSize: '0.8rem' }}>{giftCardProgress.toFixed(0)}%</span>
                  </div>
                </div>
              </div>
            )}
