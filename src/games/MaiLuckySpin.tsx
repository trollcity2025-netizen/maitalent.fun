...
const SEGMENTS = [
  ...
  { label: 'No Win', color: '#ff0080', type: 'none' },
  { label: '$0.05', color: '#00ff88', type: 'cash' },
  { label: 'Gift Card', color: '#00aaff', type: 'gift_card' },
  { label: '$0.10', color: '#ff0080', type: 'cash' },
...
export default function MaiLuckySpin() {
    ...
    setTimeout(async () => {
      ...
      if (user?.id) {
        try {
          ...
          } else if (spinResult.reward_type === 'gift_card') {
            await addGiftCardProgress(user.id, 2);
            nextMessage = '+2% Gift Card Progress!';
  ...
  }, [user, wallet?.token_balance, spinning, rotation, refreshWallet, loadTokenTransactions, addGiftCardProgress, message]);
        ...
        {result && (
          <div className="card neon-border" style={{ padding: '1rem', marginBottom: '1rem' }}>
            <p className={result.won ? 'neon-text-green' : result.reward_type === 'bankrupt' ? 'neon-text-pink' : 'neon-text-blue'} style={{ fontSize: '1.3rem', fontWeight: 'bold' }}>
              {result.reward_type === 'token' ? '+1 Free Token!' : result.reward_type === 'gift_card' ? '+2% Gift Card Progress!' : result.reward_type === 'bankrupt' ? 'BANKRUPT!' : result.won ? `You Win $${result.payout.toFixed(2)}!` : 'Try Again!'}
            </p>
          </div>
        )}
