...
function profileToWallet(profile: Profile | null): Wallet | null {
  ...
  return {
    ...
    total_won: profile.cash_balance ?? profile.total_won ?? 0,
    cash_balance: profile.cash_balance ?? profile.total_won ?? 0,
...
export const useWalletStore = create<WalletState>((set, get) => ({
  ...
  addGiftCardProgress: async (_userId, _amount) => {
    // Gift card progress was removed. Gameplay no longer adds gift card progress.
    set({ giftCardProgress: 0 });
  },

  redeemGiftCard: async (userId, amountUsd, email) => {
    const { data, error } = await supabase
      .from('gift_card_redemptions')
      .insert({ user_id: userId, amount_usd: amountUsd, gift_card_progress_used: 0, email, status: 'pending' })
      .select()
      .single();
    if (error) throw error;
    await get().loadWallet(userId);
