-- Allow service-role calls to `play_game` by only enforcing auth.uid() check when present
...
BEGIN
  -- Only enforce user ownership when auth.uid() is provided (normal client calls).
  IF auth.uid() IS NOT NULL AND auth.uid() IS DISTINCT FROM p_user_id THEN RAISE EXCEPTION 'Not allowed'; END IF;
