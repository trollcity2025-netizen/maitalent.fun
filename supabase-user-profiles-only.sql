...
CREATE TABLE IF NOT EXISTS public.user_profiles (
  ...
  tokens BIGINT NOT NULL DEFAULT 5 CHECK (tokens >= 0),
  ...
  battle_crowns BIGINT NOT NULL DEFAULT 0 CHECK (battle_crowns >= 0),
  remaining_guaranteed_turns INT NOT NULL DEFAULT 5 CHECK (remaining_guaranteed_turns >= 0),
  gift_card_progress DECIMAL(12,2) NOT NULL DEFAULT 0 CHECK (gift_card_progress >= 0),
...
BEGIN
  IF to_regclass('public.profiles') IS NOT NULL THEN
    INSERT INTO public.user_profiles (
      ...
      battle_crowns, remaining_guaranteed_turns, gift_card_progress, created_at, updated_at
    )
    SELECT
      p.id,
    ...
    ON CONFLICT (id) DO UPDATE SET
      ...
      battle_crowns = GREATEST(public.user_profiles.battle_crowns, EXCLUDED.battle_crowns),
...
BEGIN
  ...
  UPDATE public.user_profiles
  SET battle_crowns = battle_crowns - p_crowns,
      troll_coins = troll_coins + v_coins,
      updated_at = NOW()
  WHERE id = p_user_id AND battle_crowns >= p_crowns;
...
BEGIN
  ...
  SELECT tokens, remaining_guaranteed_turns
    INTO v_tokens, v_guaranteed_turns
...
CREATE POLICY "Users manage own pending paypal orders" ON public.pending_paypal_orders FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
